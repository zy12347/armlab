...
class RXArm(InterbotixRobot):
    ...
    def __init__(self, dh_config_file=None):
        ...
        # DH Params
        #self.dh_params = []    
        self.dh_params = [[0,0.10391,0,-np.pi/2],[104*np.pi/180,0,-0.20573,0],[-104*np.pi/180,0,0.200,0],[-np.pi/2,0,0,-np.pi/2],[0,0.141,0,0]]
...
#   @_ensure_initialized
    ...
    def get_ee_pose(self):
        ...
        """
        #zy' revision
        link = 5 # not sure about link's function
        joint_angles = list(self.get_joint_positions())
        #print("joint_angles",joint_angles)
        joint_angles[2] = -joint_angles[2]
        joint_angles[3] = -joint_angles[3]
        posMatrix = FK_dh(self.dh_params,joint_angles,link)
        #print("transformation_matrix",posMatrix)
        phi = np.arctan2(posMatrix[1][0],posMatrix[0][0])*R2D#roll 
        theta =  -np.arcsin(posMatrix[2][0])#pitch
        psi =  np.arctan2(posMatrix[2][1],posMatrix[2][2])*R2D#yaw
        return [posMatrix[0][3],posMatrix[1][3],posMatrix[2][3],phi,theta,psi]
...
class RXArmThread(QThread):
    ...
    def callback(self, data):
        ...
        #self.updateJointReadout.emit([0,0,0,0,0])
...
if __name__ == '__main__':
    rxarm = RXArm()#add dh_config file
